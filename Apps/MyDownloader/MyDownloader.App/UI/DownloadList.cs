...
namespace MyDownloader.App.UI
{
    public partial class DownloadList : UserControl, IClipboardDataHandler
    {
        ...
        public DownloadList()
        {
            ...
            DownloadManager.Instance.OnChange += Instance_Change;

            //for (int i = 0; i < DownloadManager.Instance.Downloads.Count; i++)
            //{
            //    AddDownload(DownloadManager.Instance.Downloads[i]);
            //}
            foreach (Downloader download in DownloadManager.Instance.OrderedDownloads)
            {
        ...
        {
            return lvwDownloads.Items.Count;
        }
        ...
        public bool HasDownloadStarted()
        {
            ...
            //{
            //    Downloader downloader = (Downloader)mapItemToDownload[item];
            ...
            //}
            //return false;
            return DownloadManager.Instance.HasDownloadStarted();
        }
        ...
        public void RemoveDownload(int i)
        {
            try
            {
                ...
                ListViewItem item = lvwDownloads.Items[i];
                DownloadAction(item,
                    delegate(Downloader d, ListViewItem item2)
                    {
                        lvwDownloads.Items.RemoveAt(item2.Index);
                        DownloadManager.Instance.RemoveDownload(d);
                    }
                );
            }
        ...
        {
            Downloader download = DownloadManager.Instance.GetDownloaderById(id);
            RemoveDownload(download.OrderedIndex);
        }
            ...
            get
            {
                if (lvwDownloads.SelectedItems.Count > 0)
                {
                    ...
                    for (int i = 0; i < downloaders.Length; i++)
                    {
                        ...
                        downloaders[i] = DownloadManager.Instance.GetOrderedDownloader(lvwDownloads.SelectedItems[i].Index);
                    }
                    return downloaders;
                }
        ...
        private void UpdateSegments()
        {
            try
            {
                ...
                if (lvwDownloads.SelectedItems.Count == 1)
                {
                    ...
                    Downloader d = DownloadManager.Instance.GetOrderedDownloader(newSelection.Index);
        ...
        private void AllDownloadsAction(ActionDownloader action)
        {
            if (lvwDownloads.Items.Count > 0)
            {
                lvwDownloads.BeginUpdate();
                lvwDownloads.ItemSelectionChanged -= new ListViewItemSelectionChangedEventHandler(lvwDownloads_ItemSelectionChanged);
                try
