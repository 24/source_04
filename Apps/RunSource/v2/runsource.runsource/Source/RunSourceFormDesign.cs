...
namespace runsourced
{
    partial class RunSourceForm
    {
        ...
        private void InitializeForm()
        {
            //InitializeMenu_v1();
            Try(CreateMenu);
            Try(SetControlsOrder);
        }

        private void SetControlsOrder()
        {
            int i = 0;
            this.Controls.SetChildIndex(this.pan_status, i++);
            this.Controls.SetChildIndex(this.tc_result, i++);
            this.Controls.SetChildIndex(this.split_top, i++);
            this.Controls.SetChildIndex(this.pan_top, i++);
            //this.Controls.SetChildIndex(this.menuStrip1, i++);
        }
        ...
        private void TraceFormControls()
        {
            ...
            _trace.WriteLine("  pan_status     : index {0}", this.Controls.IndexOf(this.pan_status));     // index 0
            _trace.WriteLine("  tc_result      : index {0}", this.Controls.IndexOf(this.tc_result));      // index 1
            _trace.WriteLine("  split_top      : index {0}", this.Controls.IndexOf(this.split_top));      // index 2
            _trace.WriteLine("  pan_top        : index {0}", this.Controls.IndexOf(this.pan_top));        // index 3
            //_trace.WriteLine("  menuStrip1     : index {0}", this.Controls.IndexOf(this.menuStrip1));     // index 4
        }
        ...
        private void CreateForm()
        {
            ...
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.RunSourceForm_KeyDown);
        ...
        private void CreateMenu()
        {
            ...
            ToolStripMenuItem m_file = zForm.CreateMenuItem("&File", false, null);
            m_file.DropDownItems.AddRange(new System.Windows.Forms.ToolStripItem[] {
                zForm.CreateMenuItem("&New (Ctrl-N)", false, m_new_Click),
                zForm.CreateMenuItem("&Open (Ctrl-O)", false, m_open_Click),
                zForm.CreateMenuItem("&Save (Ctrl-S)", false, m_save_Click),
                zForm.CreateMenuItem("Save &as (Ctrl-A)", false, m_save_as_Click),
                new ToolStripSeparator(),
                zForm.CreateMenuItem("&Execute (F5)", false, m_execute_Click),
                zForm.CreateMenuItem("Execute on &main thread (shift + F5)", false, m_execute_on_main_thread_Click),
                zForm.CreateMenuItem("Execute on &main thread (shift + F5)", false, m_execute_on_main_thread_Click),
                zForm.CreateMenuItem("Execute &without project (ctrl + F5)", false, m_execute_without_project_Click),
                zForm.CreateMenuItem("&Compile (Shift-Ctrl-B)", false, m_compile_Click),
                new ToolStripSeparator(),
                zForm.CreateMenuItem("Compile and &restart \"Run source\" (Shift-Ctrl-U)", false, m_update_runsource_Click),
                zForm.CreateMenuItem("C&ompile \"Run source\"  (Shift-Ctrl-C)", false, m_compile_runsource_Click),
                zForm.CreateMenuItem("&Restart \"Run source\"  (Shift-Ctrl-R)", false, m_restart_runsource_Click),
                new ToolStripSeparator(),
                zForm.CreateMenuItem("&Quit", false, m_quit_Click),
            });

            ToolStripMenuItem m_options = zForm.CreateMenuItem("&Options", false, null);

            m_view_source_line_number = zForm.CreateMenuItem("View source line number", true, m_view_source_line_number_Click);
            m_run_init = zForm.CreateMenuItem("Run &init", true, m_run_init_Click);
            m_allow_multiple_execution = zForm.CreateMenuItem("&Allow multiple execution", true, m_allow_multiple_execution_Click);

            m_options.DropDownItems.AddRange(new ToolStripItem[] {
                zForm.CreateMenuItem("Set grid &max width height", true, m_grid_set_max_width_height_Click),
                zForm.CreateMenuItem("Resize data table images", true, m_resize_datatable_images_Click),
            ...
            menuStrip.Items.AddRange(new ToolStripItem[] { m_file, m_options });
        ...
        private System.Windows.Forms.ToolStripTextBox status_line;
        ...
        private void CreatePanButton()
        {
            ...
            bt_execute = zForm.CreateButton("&Run", 3, 3, 75, 23, bt_execute_Click);
            bt_pause = zForm.CreateButton("&Pause", 87, 3, 75, 23, bt_pause_Click);
            bt_stop = zForm.CreateButton("&Stop", 171, 3, 75, 23, bt_stop_Click);
