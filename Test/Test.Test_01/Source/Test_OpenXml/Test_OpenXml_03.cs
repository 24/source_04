...
// from Examples\Program.cs https://github.com/WordDocX/DocX (c:\pib\dev\OpenProject\DocX)

namespace Test.Test_OpenXml
{
    public static class Test_OpenXml_03
    {
        ...
        {
            //_directory = zPath.Combine(RunSource.CurrentRunSource.ProjectDirectory, "docs\\Test_OpenXml_03");
            _directory = zPath.Combine(_directoryBase, "docs\\Test_OpenXml_03");
        ...
        {
            SetDirectory();
            string directory = zPath.Combine(_directory, "docGenerated");
        ...
        {
            Trace.WriteLine("\tBarChart()");
            // Create new document. 
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\BarChart.docx")))
            {
                // Create chart.
            ...
            Trace.WriteLine("\tCreated: docs\\BarChart.docx\n");
        }

        private static void PieChart()
        {
            Trace.WriteLine("\tPieChart()");
            // Create new document. 
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\PieChart.docx")))
            {
                // Create chart.
            ...
            Trace.WriteLine("\tCreated: docs\\PieChart.docx\n");
        }

        private static void LineChart()
        {
            Trace.WriteLine("\tLineChart()");
            // Create new document. 
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\LineChart.docx")))
            {
                // Create chart.
            ...
            Trace.WriteLine("\tCreated: docs\\LineChart.docx\n");
        }

        private static void Chart3D()
        {
            Trace.WriteLine("\tChart3D()");
            // Create new document. 
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\3DChart.docx")))
            {
                // Create chart.
                BarChart c = new BarChart();
                c.View3D = true;
            ...
            Trace.WriteLine("\tCreated: docs\\3DChart.docx\n");
        }
        ...
        private static void Contents()
        {
            ...
            // Load a document.
            using (DocX document = DocX.Load(zPath.Combine(_directory, "docGenerated\\Content.docx")))
            {
                foreach (var c in document.Contents)
                {
                    Trace.WriteLine(String.Format("Name : {0}, Tag : {1}", c.Name, c.Tag));
                }
                ...
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\ContentSetSingle.docx"));
                ...
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\ContentSetWithElement.docx"));


                doc.Save(zPath.Combine(_directory, "docGenerated\\elements.xml"));

                document.SetContent(zPath.Combine(_directory, "docGenerated\\elements.xml"));
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\ContentSetWithPath.docx"));
        ...
        {
            Trace.WriteLine("\tEquations()");

            // Create a new document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\Equations.docx")))
            {
                ...
                document.Save();
                Trace.WriteLine("\tCreated: docs\\Equations.docx\n");
            }
        }
        public static void DocumentHeading()
        {
            Trace.WriteLine("\tDocumentHeading()");
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\DocumentHeading.docx")))
            {
                ...
                document.Save();
                Trace.WriteLine("\tCreated: docs\\DocumentHeading.docx\n");
            }
        }
        ...
        {
            Trace.WriteLine("\tCreateTableFromTemplate()");

            using (DocX docX = DocX.Load(zPath.Combine(_directory, "docSource\\DocumentWithTemplateTable.docx")))
            {
                ...
                if (orderTable != null)
                {
                    ...
                    docX.SaveAs(zPath.Combine(_directory, "docGenerated\\CreateTableFromTemplate.docx"));

                }
                else
                {
                    Trace.WriteLine("\tError, couldn't find table with caption ORDER_TABLE in document");
                }


            }
            Trace.WriteLine("\tCreated: docs\\CreateTableFromTemplate.docx");
        }

        private static void Bookmarks()
        {
            Trace.WriteLine("\tBookmarks()");

            using (var document = DocX.Create(zPath.Combine(_directory, "docGenerated\\Bookmarks.docx")))
            {
                ...
                document.Save();
                Trace.WriteLine("\tCreated: docs\\Bookmarks.docx\n");
        ...
        {
            Trace.WriteLine("\tBookmarksReplaceTextOfBookmarkKeepingFormat()");

            using (DocX docX = DocX.Load(zPath.Combine(_directory, "docSource\\DocumentWithBookmarks.docx")))
            {
                foreach (Bookmark bookmark in docX.Bookmarks)
                    Trace.WriteLine("\t\tFound bookmark {0}", bookmark.Name);
                ...
                docX.SaveAs(zPath.Combine(_directory, "docGenerated\\BookmarksReplaceTextOfBookmarkKeepingFormat.docx"));
            }
            Trace.WriteLine("\tCreated: docs\\BookmarksReplaceTextOfBookmarkKeepingFormat.docx");
        }
        ...
        private static void Indentation()
        {
            ...
            // Create a new document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\Indentation.docx")))
            {
                ...
                document.Save();
                Trace.WriteLine("\tCreated: docs\\Indentation.docx\n");
            }
        }
        ...
        {
            Trace.WriteLine("\tRightToLeft()");

            // Create a new document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\RightToLeft.docx")))
            {
                ...
                // Save all changes made to this document.
                document.Save();
                Trace.WriteLine("\tCreated: docs\\RightToLeft.docx\n");
            }
        }
        ...
        {
            Trace.WriteLine("\tHyperlinksImagesTables()");

            // Create a document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HyperlinksImagesTables.docx")))
            {
                ...
                Trace.WriteLine("\tCreated: docs\\HyperlinksImagesTables.docx\n");
            }
        }
        private static void HyperlinksImagesTablesWithLists()
        {
            Trace.WriteLine("\tHyperlinksImagesTablesWithLists()");

            // Create a document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HyperlinksImagesTablesWithLists.docx")))
            {
                // Add a hyperlink into the document.
                Hyperlink link = document.AddHyperlink("link", new Uri("http://www.google.com"));
                ...
                Trace.WriteLine("\tCreated: docs\\HyperlinksImagesTablesWithLists.docx\n");
            }
        }

        private static void DocumentMargins()
        {
            Trace.WriteLine("\tDocumentMargins()");

            // Create a document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\DocumentMargins.docx")))
            {
                ...
                Trace.WriteLine("\tCreated: docs\\DocumentMargins.docx\n");
            }
        }

        private static void DocumentsWithListsFontChange()
        {
            Trace.WriteLine("\tDocumentsWithListsFontChange()");

            // Create a document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\DocumentsWithListsFontChange.docx")))
            {
                ...
                Trace.WriteLine("\tCreated: docs\\DocumentsWithListsFontChange.docx\n");
            }
        }
        ...
        private static void AddList()
        {
            ...
            using (var document = DocX.Create(zPath.Combine(_directory, "docGenerated\\Lists.docx")))
            {
                ...
                document.InsertList(bulletedList);
                document.Save();
                Trace.WriteLine("\tCreated: docs\\Lists.docx");
            }
        }

        private static void HeadersAndFooters()
        {
            Trace.WriteLine("\tHeadersAndFooters()");

            // Create a new document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HeadersAndFooters.docx")))
            {
                ...
                Trace.WriteLine("\tCreated: docs\\HeadersAndFooters.docx\n");
            }// Release this document from memory.
        }
        private static void HeadersAndFootersWithImagesAndTables()
        {
            Trace.WriteLine("\tHeadersAndFootersWithImagesAndTables()");

            // Create a new document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HeadersAndFootersWithImagesAndTables.docx")))
            {
                ...
                Trace.WriteLine("\tCreated: docs\\HeadersAndFootersWithImagesAndTables.docx\n");
            }// Release this document from memory.
        }
        private static void HeadersAndFootersWithImagesAndTablesUsingInsertPicture()
        {
            Trace.WriteLine("\tHeadersAndFootersWithImagesAndTablesUsingInsertPicture()");

            // Create a new document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HeadersAndFootersWithImagesAndTablesUsingInsertPicture.docx")))
            {
                // Add a template logo image to this document.
                //RelativeDirectory rd = new RelativeDirectory(); // prepares the files for testing
                //rd.Up(2);
                ...
                Trace.WriteLine("\tCreated: docs\\HeadersAndFootersWithImagesAndTablesUsingInsertPicture.docx\n");
            }// Release this document from memory.
        }

        private static void CreateInvoice()
        {
            Trace.WriteLine("\tCreateInvoice()");
            DocX g_document;

            try
            {
                // Store a global reference to the loaded document.
                g_document = DocX.Load(zPath.Combine(_directory, "docGenerated\\InvoiceTemplate.docx"));
                 ...
                 */
                g_document = CreateInvoiceFromTemplate(DocX.Load(zPath.Combine(_directory, "docGenerated\\InvoiceTemplate.docx")));

                // Save all changes made to this template as Invoice_The_Happy_Builder.docx (We don't want to replace InvoiceTemplate.docx).
                g_document.SaveAs(zPath.Combine(_directory, "docGenerated\\Invoice_The_Happy_Builder.docx"));
                Trace.WriteLine("\tCreated: docs\\Invoice_The_Happy_Builder.docx\n");
            }
            ...
            catch (FileNotFoundException)
            {
                ...
                g_document.Save();
                Trace.WriteLine("\tCreated: docs\\InvoiceTemplate.docx");
        ...
        {
            Trace.WriteLine("\tCreateTableWithTextDirection()");

            // Create a document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\CeateTableWithTextDirection.docx")))
            {
            ...
            Trace.WriteLine("\tCreated: docs\\CreateTableWithTextDirection.docx");
        }
        ...
        private static DocX CreateInvoiceFromTemplate(DocX template)
        {
            ...
            Image logo = template.AddImage(zPath.Combine(_directory, @"images\logo_the_happy_builder.png"));
        ...
        {
            // Create a new document.
            DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\InvoiceTemplate.docx"));
            ...
            Image logo = document.AddImage(zPath.Combine(_directory, @"images\logo_template.png"));
        ...
        {
            Trace.WriteLine("\tHelloWorld()");

            // Create a new document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HelloWorld.docx")))
            {
                ...
                // Save this document to disk.
                document.Save();
                Trace.WriteLine("\tCreated: docs\\HelloWorld.docx\n");
            }
        }

        static void HelloWorldInsertHorizontalLine()
        {
            Trace.WriteLine("\tHelloWorldInsertHorizontalLine()");

            // Create a new document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HelloWorldInsertHorizontalLine.docx")))
            {
                // Insert a Paragraph into this document.
                Paragraph p = document.InsertParagraph();
                ...
                // Save this document to disk.
                document.Save();
                Trace.WriteLine("\tCreated: docs\\HelloWorldInsertHorizontalLine.docx\n");
            }
        }

        static void HelloWorldProtectedDocument()
        {
            Trace.WriteLine("\tHelloWorldPasswordProtected()");

            // Create a new document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HelloWorldPasswordProtected.docx")))
            {
                // Insert a Paragraph into this document.
                Paragraph p = document.InsertParagraph();
                ...
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\HelloWorldPasswordProtectedReadOnly.docx"));
                Trace.WriteLine("\tCreated: docs\\HelloWorldPasswordProtectedReadOnly.docx\n");
                ...
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\HelloWorldPasswordProtectedCommentsOnly.docx"));
                Trace.WriteLine("\tCreated: docs\\HelloWorldPasswordProtectedCommentsOnly.docx\n");
                ...
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\HelloWorldPasswordProtectedFormsOnly.docx"));
                Trace.WriteLine("\tCreated: docs\\HelloWorldPasswordProtectedFormsOnly.docx\n");
                ...
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\HelloWorldPasswordProtectedTrackedChangesOnly.docx"));
                Trace.WriteLine("\tCreated: docs\\HelloWorldPasswordProtectedTrackedChangesOnly.docx\n");

                // But it's also possible to add restrictions without protecting it with password.

                // Protected with password for Read Only
                document.AddProtection(erReadOnly);
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\HelloWorldWithoutPasswordReadOnly.docx"));
                Trace.WriteLine("\tCreated: docs\\HelloWorldWithoutPasswordReadOnly.docx\n");

                // Protected with password for Comments
                document.AddProtection(erComments);
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\HelloWorldWithoutPasswordCommentsOnly.docx"));
                Trace.WriteLine("\tCreated: docs\\HelloWorldWithoutPasswordCommentsOnly.docx\n");

                // Protected with password for Forms
                document.AddProtection(erForms);
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\HelloWorldWithoutPasswordFormsOnly.docx"));
                Trace.WriteLine("\tCreated: docs\\HelloWorldWithoutPasswordFormsOnly.docx\n");

                // Protected with password for Tracked Changes
                document.AddProtection(erTrackedChanges);
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\HelloWorldWithoutPasswordTrackedChangesOnly.docx"));
                Trace.WriteLine("\tCreated: docs\\HelloWorldWithoutPasswordTrackedChangesOnly.docx\n");
            }
        }

        static void HelloWorldAdvancedFormatting()
        {
            Trace.WriteLine("\tHelloWorldAdvancedFormatting()");
            // Create a document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HelloWorldAdvancedFormatting.docx")))
            {
                ...
                // Save this document.
                document.Save();
                Trace.WriteLine("\tCreated: docs\\HelloWorldAdvancedFormatting.docx\n");
            }// Release this document from memory.
        }
        ...
        {
            Trace.WriteLine("\tProgrammaticallyManipulateImbeddedImage()");
            const string str = "Hello World";

            // Open the document Input.docx.
            using (DocX document = DocX.Load(zPath.Combine(_directory, "docSource\\Input.docx")))
            {
                ...
                if (document.Images.Count() > 0)
                {
                    ...
                    Trace.WriteLine("The provided document contains no Images.");

                // Save this document as Output.docx.
                document.SaveAs(zPath.Combine(_directory, "docGenerated\\Output.docx"));
                Trace.WriteLine("\tCreated: docs\\Output.docx\n");
            }
        }
        ...
        {
            Trace.WriteLine("\tReplaceTextParallel()\n");
            ...
            Trace.WriteLine("\tCreated: None\n");
        }

        static void AddToc()
        {
            Trace.WriteLine("\tAddToc()");

            using (var document = DocX.Create(zPath.Combine(_directory, "docGenerated\\Toc.docx")))
            {
        ...
        {
            Trace.WriteLine("\tAddTocByReference()");

            using (var document = DocX.Create(zPath.Combine(_directory, "docGenerated\\TocByReference.docx")))
            {
                var h1 = document.InsertParagraph("Heading 1");
                h1.StyleName = "Heading1";
                document.InsertParagraph("Some very interesting content here");
        ...
        static void HelloWorldKeepWithNext()
        {
            ...
            Trace.WriteLine("\tHelloWorldKeepWithNext()");
            // Create a new document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HelloWorldKeepWithNext.docx")))

            {
                ...
                // Save all changes made to this document.
                document.Save();
                Trace.WriteLine("\tCreated: docs\\HelloWorldKeepWithNext.docx\n");
            }
        }
        static void HelloWorldKeepLinesTogether()
        {
            // Create a Paragraph that will stay on the same page as the paragraph that comes next
            Trace.WriteLine("\tHelloWorldKeepLinesTogether()");
            // Create a new document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HelloWorldKeepLinesTogether.docx")))
            {
                // Create a new Paragraph with the text "Hello World".
                ...
                // Save all changes made to this document.
                document.Save();
                Trace.WriteLine("\tCreated: docs\\HelloWorldKeepLinesTogether.docx\n");
            }
        }

        static void LargeTable()
        {
            Trace.WriteLine("\tLargeTable()");
            var directory = _directory + "\\docGenerated\\";
            using (var output = File.Open(directory + "LargeTable.docx", FileMode.Create))
            {
            ...
            Trace.WriteLine("\tCreated: docs\\LargeTable.docx\n");
        }


        static void TableWithSpecifiedWidths()
        {
            Trace.WriteLine("\tTableSpecifiedWidths()");
            var directory = _directory + "\\docGenerated\\";
            using (var output = File.Open(directory + "TableSpecifiedWidths.docx", FileMode.Create))
            {
                using (var doc = DocX.Create(output))
                {
            ...
            Trace.WriteLine("\tCreated: docs\\TableSpecifiedWidths.docx\n");
        ...
        {
            Trace.WriteLine("\tHelloWorldAddPictureToWord()");

            // Create a document.
            using (DocX document = DocX.Create(zPath.Combine(_directory, "docGenerated\\HelloWorldAddPictureToWord.docx")))
            {
                // Add an image into the document.    
                //RelativeDirectory rd = new RelativeDirectory(); // prepares the files for testing
                //rd.Up(2);
                ...
                // Save this document.
                document.Save();
                Trace.WriteLine("\tCreated: docs\\HelloWorldAddPictureToWord.docx\n");
            }
        }
    }
}
